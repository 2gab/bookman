...
- **Projeção mecânica** (Metadados, Path) — assembled from canonical state with no interpretation involved (rendering a table as links, reading a field back). Always computed fresh, never persisted — there's nothing to get wrong by recomputing it every time.
...
Not building referência yet. It's a genuinely different shape of interaction from prática (consult, don't attempt-and-correct) — the next real test for prática is running it against actual exercises in a live session (`basic_mathematics.pdf`, Serge Lang, is the first real case), the same way "Creating a cartão" and "discovery" were proven before being trusted.
