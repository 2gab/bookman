...
- Never call something Contexto's answer without its `[CONTEXTO EXTERNO — <fonte>]` tag, and never let Contexto tell the reader their own interpretation is wrong — name a tension between the reader's carta and an external source, don't resolve it for them. Never collapse two disagreeing external sources into one merged voice. Never run Contexto automatically — reader-invoked only, same as Referência, cartão, prática, and discovery. Never confuse Contexto with Referência: Contexto only ever draws from outside the primary text, Referência only ever from inside it.
